    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${navBg}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link to="/" className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isLanding && !scrolled ? 'bg-white/20' : 'bg-blue-600'}`}>
              <Zap size={18} className={isLanding && !scrolled ? 'text-white' : 'text-white'} />
            </div>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 flex flex-col gap-3">
          <Link to="/" className="text-sm font-medium text-gray-700 py-2" onClick={() => setMobileOpen(false)}>Beranda</Link>
          <a href="#features" className="text-sm font-medium text-gray-700 py-2" onClick={() => setMobileOpen(false)}>Fitur</a>
          <a href="#examples" className="text-sm font-medium text-gray-700 py-2" onClick={() => setMobileOpen(false)}>Contoh</a>
          <Link
            to="/generate"
