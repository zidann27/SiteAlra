...
export default function CTASection() {
  ...
  return (
    <section className="py-24 bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 relative overflow-hidden transition-colors duration-300">
      ...
      <div
        ref={ref}
        style={revealStyle(visible, 0, 700, 36)}
        className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center"
      >
        {/* Stars */}
        <div
          style={{ opacity: visible ? 1 : 0, transition: 'opacity 600ms ease', transitionDelay: '100ms' }}
        ...
        <p style={revealStyle(visible, 250)} className="text-xl text-slate-300 mb-10 max-w-2xl mx-auto leading-relaxed">
        ...
        <div style={revealStyle(visible, 350)}>
          <SmartCTALink className="inline-flex items-center gap-2 px-10 py-5 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl transition-all duration-200 shadow-xl shadow-blue-600/25 hover:-translate-y-0.5 text-lg">
            Mulai Sekarang — Gratis
            <ArrowRight size={20} />
          </SmartCTALink>
          <p style={{ opacity: visible ? 1 : 0, transition: 'opacity 800ms ease', transitionDelay: '500ms' }} className="mt-6 text-slate-400 text-sm">
            Tidak perlu kartu kredit · Tidak perlu coding · Siap dalam 30 detik
          </p>
        </div>
      </div>
    </section>
