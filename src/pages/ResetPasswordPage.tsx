  ...
  return (
    <div className="min-h-screen w-full flex flex-col md:flex-row font-sans selection:bg-blue-100 bg-white">
      ...
      <div className="hidden md:flex md:w-1/2 bg-[#020617] items-center justify-center p-16 min-h-screen relative overflow-hidden">
        <div className="absolute inset-0 z-0 opacity-25 bg-cover bg-center" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1555421689-d68471e189f2?auto=format&fit=crop&q=80')" }} />
        ...
        <div className="relative z-10 w-full max-w-md flex flex-col justify-center">
          ...
          <p className="text-zinc-400 text-sm leading-relaxed max-w-xs">
            Buat password baru yang kuat untuk mengamankan akun kamu. Pastikan kamu mengingatnya dengan baik.
          </p>
          <div className="mt-10 space-y-3">
            {["Minimal 6 karakter", "Kombinasi huruf besar & kecil", "Tambahkan angka atau simbol"].map((tip) => (
      ...
      <div className="w-full md:w-1/2 bg-white flex flex-col min-h-screen relative">
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-10 sm:px-10">
          <div className="w-full max-w-sm">
            ...
            <div className="mb-8">
              <h2 className="text-3xl sm:text-4xl font-bold text-zinc-900 mb-3">Buat Password Baru</h2>
              <p className="text-zinc-500 text-sm">Masukkan password baru untuk akun kamu.</p>
            </div>
            ...
            {done ? (
              ...
              <form onSubmit={onSubmit} className="space-y-5">
                ...
                {/* Password baru */}
                <div>
                  <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">Password Baru</label>
                  ...
                  {password.length > 0 && (
                    <div className="mt-2 space-y-1">
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((i) => (
                          <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= strength ? strengthColor : "bg-zinc-100"}`} />
                        ))}
                      </div>
                      <p className={`text-xs font-medium ${strength <= 1 ? "text-red-500" : strength === 2 ? "text-orange-500" : strength === 3 ? "text-yellow-600" : strength === 4 ? "text-blue-600" : "text-emerald-600"}`}>
                        {strengthLabel}
                      </p>
                    </div>
                  )}
                ...
                {/* Konfirmasi password */}
                <div>
                  <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">Konfirmasi Password</label>
                  <div className="relative group flex items-center">
                    <div className="absolute left-4 flex items-center justify-center">
                      <Lock size={18} className="text-zinc-400 group-focus-within:text-blue-600 transition-colors" />
                    </div>
                    <input
                      ...
                      className={`w-full pl-12 pr-12 py-4 bg-zinc-50/50 hover:bg-white border rounded-2xl focus:outline-none focus:bg-white focus:ring-4 transition-all text-zinc-900 placeholder-zinc-400 text-sm font-medium tracking-wide ${
                        confirm.length > 0 && confirm !== password ? "border-red-200 focus:border-red-300 focus:ring-red-600/5" :
                        confirm.length > 0 && confirm === password ? "border-emerald-200 focus:border-emerald-300 focus:ring-emerald-600/5" :
                        "border-zinc-100 focus:border-blue-200 focus:ring-blue-600/5"
                      }`}
                    />
                    <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-4 text-zinc-400 hover:text-blue-600 transition-colors focus:outline-none" tabIndex={-1}>
